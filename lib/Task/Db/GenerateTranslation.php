 ...
 */
class Task_Db_GenerateTranslation extends Ruckusing_Task_Base implements Ruckusing_Task_Interface
{
    /**
     * Current Adapter
     *
...
class $klass extends Ruckusing_Migration_Base
{
    ...
    public function up()
    {
        foreach (\$this->language_keys as \$key => \$value) {
            \$this->execute("INSERT INTO `translations` (`key`, `en_US`) VALUES('".\$this->quote_string(\$key)."', '". \$this->quote_string(\$value) ."')
                ON DUPLICATE KEY UPDATE `en_US`= '". \$this->quote_string(\$value) ."';");
        }
        // foreach (\$this->language_keys_es as \$key => \$value) {
        //     \$this->execute("INSERT INTO `translations` (`key`, `es_ES`) VALUES('".\$this->quote_string(\$key)."', '". \$this->quote_string(\$value) ."')
        //         ON DUPLICATE KEY UPDATE `es_ES`= '". \$this->quote_string(\$value) ."';");
        // }
    }//up()
    ...
    {
        foreach (\$this->language_keys as \$key => \$value) {
            \$this->execute("DELETE FROM `translations` WHERE `key` = '\$key'");
        }
    }//down()
}
...
\tTask: db:generatetranslation <migration name>

\tGenerator for migrations with translations.
